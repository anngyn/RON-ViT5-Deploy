...
\subtitle{Báo cáo cuối kỳ môn Chuyên đề Xử lý Ngôn ngữ Tự nhiên}
\author{Nhóm 19}
...
\begin{block}{Thực nghiệm chính}
\textbf{VietAI/ViT5-base}\\
Clean, Noisy Aug, Consistency\\
Full data, 3 epochs, LR $5 \times 10^{-5}$\\
Đánh giá 14 nhiễu tại L2
\end{block}
\end{column}
\begin{column}{0.49\textwidth}
\begin{block}{Pilot cross-backbone}
\textbf{google/mT5-base}\\
\textbf{vinai/BARTpho-syllable-base}\\
Baseline clean, severity L1--L3\\
Retention và noise-ranking
\end{block}
\end{column}
\end{columns}
...
ReceiptVQA, cùng test split \quad ANLS@0.5\\
Input/output: 256/64 \quad beam $k=4$ \quad synthetic OCR noise, seed 42
\end{alertblock}

\scriptsize ViT5 so sánh phương pháp tại L2; pilot kiểm tra pattern lỗi giữa backbone, không xếp hạng model.

\end{frame}

\section{Phạm vi và kết quả thực nghiệm}

\begin{frame}
\frametitle{Phạm vi đánh giá và đầu ra}

\begin{center}
\textbf{3 flow ViT5 \qquad 14 nhiễu \qquad 2 backbone pilot}
\end{center}

\vspace{0.45cm}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.48\textwidth}
\begin{block}{Phạm vi đánh giá}
Dataset và loader\\
Taxonomy nhiễu\\
...
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Đầu ra báo cáo}
CSV cho 3 flow\\
Ranking noise\\
Biểu đồ recovery\\
Severity mT5/BARTpho
\end{block}
\end{column}
\end{columns}
...
\section{So sánh backbone pilot}

\begin{frame}
\frametitle{Độ bền giảm khi severity tăng}

\begin{center}
\includegraphics[width=0.80\textwidth,height=0.46\textheight,keepaspectratio]{img/fig_backbone_severity_retention.png}
\end{center}

\begin{alertblock}{Kết quả pilot trên backbone khác}
\small
mT5 giữ lại $98.0\% \rightarrow 96.2\% \rightarrow 94.0\%$ từ L1 đến L3. BARTpho giữ lại $93.9\% \rightarrow 90.3\% \rightarrow 86.4\%$. Cả hai đều giảm khi severity tăng; BARTpho giảm tương đối mạnh hơn.
\end{alertblock}

\scriptsize Đây là kiểm tra tính lặp lại của pattern vulnerability, không phải cuộc thi chọn backbone thắng.

\end{frame}

\begin{frame}
\frametitle{Noise chi phối và tính ổn định giữa backbone}

\begin{columns}[T,onlytextwidth]
\begin{column}{0.58\textwidth}
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{img/fig_backbone_l3_noise_drop.png}
\end{column}
\begin{column}{0.39\textwidth}
\begin{block}{Spearman noise-ranking}
\small
L1: $\rho=0.899$\\
L2: $\rho=0.881$\\
L3: $\rho=0.873$
...
\scriptsize Mixed noise đứng đầu và money noise đứng thứ hai ở L3. N1 bỏ dấu được tách riêng vì không có severity thực; N20 là stress test mixed.
...
\end{frame}
\begin{frame}
\frametitle{Giới hạn và hướng phát triển}
...
\begin{alertblock}{Giới hạn của kết luận}
...
\textbf{Ngoài phạm vi báo cáo:} sẽ dùng cho thí nghiệm mở rộng sau khi hoàn tất equal-budget run và confidence interval.
...
\begin{enumerate}
    \item Benchmark full-data trên ReceiptVQA với ViT5
    \item 14 nhiễu OCR có kiểm soát tại L2 và pilot severity trên mT5/BARTpho
    \item Mixed/money chi phối suy giảm; noise ranking ổn định giữa backbone pilot
    \item Noisy Aug đạt ANLS tuyệt đối cao nhất trong thiết lập ViT5 đã thử
\end{enumerate}

\begin{block}{Thông điệp chính}
Đánh giá độ bền OCR có kiểm soát; backbone pilot dùng để kiểm tra tính lặp lại của vulnerability pattern.
\end{block}
