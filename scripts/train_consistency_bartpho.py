"""Train with paired clean/noisy data + consistency loss, any seq2seq backbone.

Copy of train_consistency.py with T5-specific tokenizer/model classes swapped
...
def main(config_path, skip_final_eval=False, batch_size_override=None):
    ...
    logging.info("=" * 60)
    logging.info(f"TRAINING: Consistency Only ({config['model_name']})")
    logging.info("=" * 60)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train with consistency loss (generic seq2seq backbone).")
    parser.add_argument(
        '--config',
        default=os.path.join(os.path.dirname(__file__), '..', 'configs', 'consistency_bartpho.yaml'),
