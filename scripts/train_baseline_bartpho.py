"""Train baseline on clean data with any seq2seq backbone (BARTpho/mT5/...).

Copy of train_baseline.py with T5-specific tokenizer/model classes swapped
for the generic Auto* classes so it works with BARTpho and other
encoder-decoder backbones without touching the original ViT5 script/results.
"""
import argparse
import os
import sys
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train baseline on clean data (generic seq2seq backbone).")
    parser.add_argument(
        '--config',
        default=os.path.join(os.path.dirname(__file__), '..', 'configs', 'baseline_bartpho.yaml'),
