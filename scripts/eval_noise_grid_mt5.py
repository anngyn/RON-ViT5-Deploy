"""Evaluate a trained model (any seq2seq backbone) on clean + OCR noise grid.

Copy of eval_noise_grid.py with T5-specific tokenizer/model classes swapped
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate model on ReceiptVQA OCR noise grid (generic backbone).")
    ...
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_source, legacy=True)
